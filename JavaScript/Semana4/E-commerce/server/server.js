...
mercadopago.configure({
	//configurar el paquete token de acceso
	access_token: '',
});
...
app.get('/', function (req, res) {
	//definir una ruta para la url que envea el archivo index.html
...
app.post('/create_preference', (req, res) => {
	//definir una ruta para crear una preferencia de pago
	...
	let preference = {
		...
		back_urls: {
			//cuando la compra se realiza con exito redirige al servidor
	...
	mercadopago.preferences
		...
		.then(function (response) {
			res.json({
				//Nos devuelve una repuesta en forma de json
				id: response.body.id,
			});
		})
...
app.get('/feedback', function (req, res) {
	//definir una ruta para manejar los comentarios del procesamiento de pagos
	res.json({
...
app.listen(8080, () => {
	//Escucha el servidor en el puerto 8080
